...
# PROMPT: Write a retail CV event emitter that validates and serialises
# detection events to JSONL format matching the required schema.
# Schema fields: event_id, store_id, camera_id, visitor_id, event_type,
# timestamp, zone_id, dwell_ms, is_staff, confidence, metadata.
# CHANGES MADE: Added BILLING_QUEUE_JOIN auto-detection when queue_depth > 2,
# added schema validation before emit, used UUID4 for event_id.
"""
import json
import uuid
...
def make_event(
    ...
    """
    Build a validated event dict matching the required schema.
    Automatically upgrades ZONE_ENTER to BILLING_QUEUE_JOIN
    when queue_depth > 2 in the billing zone.
    """
